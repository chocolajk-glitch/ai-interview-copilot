...
def test_code_block_not_sliced():
    """长代码块允许被按行切，关键是不丢内容 + 每段都带围栏。

    设计动机（BGE 512 token 上限）：超长代码必须切，否则 embedding 截断。
    splitter 会在 chunk_size * 1.1 阈值处切，并对每段补回开闭 fence，
    保证下游 LLM/embedding 仍识别为完整代码块。
    """
    long_code_line = "    x = 1\n" * 200
    text = (
        f"## 题目\n代码如下：\n"
    ...
    # 必切：200 行 * 10 字符 = 2000 字符 > 阈值 220
    assert len(code_chunks) > 1, "长代码块必须被按行切"
    # 每段都得带开闭 fence（保留代码块语义）
    for c in code_chunks:
        assert c.page_content.lstrip().startswith("```"), (
            f"代码段丢失开围栏: {c.page_content[:30]!r}"
        )
        assert c.page_content.rstrip().endswith("```"), (
            f"代码段丢失闭围栏: {c.page_content[-30:]!r}"
        )
    # 拼回后总行数 = 原始 200 行代码（overlap 算多几行无妨，但不能少）
    total_code_lines = sum(
        c.page_content.count("\n") for c in code_chunks
    ) - 2 * len(code_chunks)  # 减去每段的 fence 行
    assert total_code_lines >= 200, (
        f"代码行丢失: 期望 ≥200，实际 {total_code_lines}"
    )
