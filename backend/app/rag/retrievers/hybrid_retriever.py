...
from app.rag.retrievers.vector_retriever import expand_to_parents, similarity_search


class HybridRetriever:
    """BM25 + 向量混合检索，RRF 融合 + Rerank 精排 + Parent-Child 展开。"""
    ...
    def search(
        ...
        """混合检索：BM25 + 向量 → RRF → Rerank → (parent 展开) → top_k。
        ...
        Args:
            ...
            expand_parent: 是否把 child 展开为 parent（默认 True）。
                关掉时直接返回 Rerank/RRF 后的 child。
        """
        # 1. BM25 召回
        ...
        if use_rerank and len(rrf_top_docs) > top_k:
            ...
            rrf_top_docs = rerank(query, rrf_top_docs, top_k=top_k)

        # 6. Parent-Child 展开：child → parent，去重后取 top_k
        if expand_parent:
            expanded = expand_to_parents(rrf_top_docs)
